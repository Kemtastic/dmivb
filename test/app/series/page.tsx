...
export default function SeriesPage() {
  ...
  useEffect(() => {
    const loadSeries = async () => {
      try {
        setLoading(true);
        const seriesData = await fetchContentsByType(ContentType.DIZI, 'releaseYear', 'desc');
        // Convert string dates to Date objects
        const seriesWithDates = seriesData.map((item: any) => ({
          ...item,
          createdAt: new Date(item.createdAt),
          updatedAt: new Date(item.updatedAt),
        }));
        setSeries(seriesWithDates);
      } catch (err) {
