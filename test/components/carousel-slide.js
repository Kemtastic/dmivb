...
export default function CarouselSlide() {
  ...
  return (
    <div className="bg-[#161515] min-h-screen p-8">
      <h2 className="text-white text-base font-bold mb-4 font-sans">Trending Now</h2>
      ...
      >
        {isHovered && currentIndex > 0 && (
          <button className="absolute h-full z-10 bg-black/50 text-white border-none cursor-pointer p-4 transition-colors duration-300 hover:bg-black/70 left-0 rounded-tr rounded-br" onClick={handlePrev}>
            <ChevronLeft size={24} />
          </button>
        )}

        {isHovered && currentIndex < movies.length - 5 && (
          <button className="absolute h-full z-10 bg-black/50 text-white border-none cursor-pointer p-4 transition-colors duration-300 hover:bg-black/70 right-0 rounded-tl rounded-bl" onClick={handleNext}>
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/70 to-black/40 flex items-end opacity-0 transition-opacity duration-300 rounded group-hover:opacity-100">
                      <div className="p-4 w-full">
                        <div className="flex justify-between items-center mb-1">
                          <h3 className="text-white text-xs font-semibold leading-tight tracking-wide">{movie.title}</h3>
                          <div className="flex items-center">
                            <span className="text-white text-xs mr-1">{movie.score}</span>
